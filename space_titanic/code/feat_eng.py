...
def feat_eng(df, train=False):
    ...
    df.loc[df['CryoSleep'] == True, cols] = df.loc[df['CryoSleep'] == True, cols].fillna(0)
    ...
    if train:
        ...
        categorical_feature_names = preprocessor.transformers_[1][1].get_feature_names(input_features=categorical_cols)
    ...
    # Convert the transformed data to a DataFrame with the new column names
        ...
        return X_train_preprocessed, X_test_preprocessed, y_train, y_test, X_transformed
